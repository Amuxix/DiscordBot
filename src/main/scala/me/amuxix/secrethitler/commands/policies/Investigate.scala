...
object Investigate extends SecretHitlerTextCommand {
  override def pattern: Regex = s"^investigate ${Command.userID}$$".r
    ...
    event.content.toLowerCase match {
      case regex(id) =>
        event.jda.getUserByID(id.toLong).flatMap { target =>
          game match {
            case game: WaitingForPolicyResolution => game.investigate(target)
